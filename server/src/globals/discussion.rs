...
pub async fn get(mut db: PooledConnection<ConnectionManager<MysqlConnection>>, id: Option<String>, nonce: Option<Vec<String>>, project_id: String) -> (PooledConnection<ConnectionManager<MysqlConnection>>, Vec<Discussions_public>, std::collections::HashMap<String, i64>, Vec<Accounts_public>) {
    ...
    let mut discussions_distinct_authors_public: Vec<Accounts_public> = discussions_distinct_authors
    .into_iter()
    .map(|(account)| {
        Accounts_public::from((account.unwrap()))
    })
    .collect();
