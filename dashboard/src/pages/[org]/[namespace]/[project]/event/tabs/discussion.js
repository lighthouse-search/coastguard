...
import Ratings_button from '@/components/internal_components/user_rating/user_rating';
...
export default function Discussion(page_props) {
    ...
    useEffect(() => {
        ...
        if (page_props.discussion) {
            set_discussion(page_props.discussion);
            return;
        }

        get_discussion(page_props, router, set_discussion);
    });
            ...
            <Lettre {...props}>
                <div className='discussion_post column row_gap_4'>
                    ...
                    <div className='row space_between'>
                        <Account_nameplate data={data.author}/>
                    ...
                    <Interaction_bottom_bar id={"discussion-"+data.message_id} user_rating={page_props.user_rating} set_reply_enabled={set_reply_enabled} on_user_rating_update={page_props.on_user_rating_update}/>
                </div>
            </Lettre>
        )
    });
    ...
    return (
        <div className='row align_items_start column_gap_6'>
            <div className='lettre_ul width_100'>
                ...
                <Reply_component discussion_id={page_props.discussion_id} on_update={() => { get_discussion({ discussion_id: page_props.discussion_id }, router, set_discussion); }}/>
            </div>
