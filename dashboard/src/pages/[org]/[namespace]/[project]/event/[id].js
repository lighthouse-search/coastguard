...
export default function Event() {
    ...
    async function run() {
        // new thread
        const data = await Coastguard(await credentials_object(router)).event.list([], { nonce_hash: [router.query.id], user_rating: true });
    ...
    return (
        <Home1 header={["event", router.query.id]} className="event" style={{ padding: 12, paddingTop: 8 }}>
            <Edit_dialog/>
            {/* padding (outer) for this page should be expanded, from 6px to 8px on home1_children_content. */}
            <div className='topbar'>
                <h1 className='title'>{events.data[0].content.slice(0, 400)}</h1>
                ...
                <div className='row space_between'>
                    <div className='row column_gap_6'>
                        ...
                        <Button_with_icon icon="/icons/explosion-solid.svg" className={`tab_button ${tab == "events" && 'tab_button_selected'}`} onClick={() => { set_tab("events"); }}>Events <span className='no_bold greyText'>{events && `${events.total}`}</span></Button_with_icon>
            ...
            <div className='content'>
                ...
                {tab == "events" && <Error_Events event={events} discussion={event_discussion} user_rating={event_user_rating} on_user_rating_update={() => { get_event_user_ratings(events.data.map(event => "event-"+event.id, set_event_user_rating)); }} jump_to={jump_to} set_jump_to={set_jump_to} tab={tab} set_tab={set_tab}/>}
                {tab == "discussion" && <Error_Discussion discussion_id={`events-${events.data[0].nonce_hash}`} discussion={discussion} user_rating={discussion_user_rating} on_user_rating_update={() => { get_event_user_ratings(discussion.data.map(event => "discussion-"+event.message_id), set_discussion_user_rating) }} jump_to={jump_to} set_jump_to={set_jump_to} tab={tab} set_tab={set_tab}/>}
            </div>
        </Home1>
    )
}